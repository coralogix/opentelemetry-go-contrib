...
func (i *instrumentor) tracingBegin(ctx context.Context, eventJSON []byte, event ...interface{}) (context.Context, trace.Span, trace.Span) {
	...
	attributes, eventSpanKind, eventSpanName, eventAttributes := spanNameAndAttributesFromEvent(event, attributes)
	if eventSpanName != "" {
		ctx, eventSpan = i.tracer.Start(ctx, eventSpanName, trace.WithSpanKind(eventSpanKind), trace.WithAttributes(eventAttributes...))
	}

	ctx, span = i.tracer.Start(ctx, spanName, trace.WithSpanKind(trace.SpanKindServer), trace.WithAttributes(attributes...))

	return ctx, span, eventSpan
}

// Logic to wrap up OTel Tracing.
func (i *instrumentor) tracingEnd(ctx context.Context, span trace.Span, eventSpan trace.Span) {
	span.End()
	if eventSpan != nil {
		eventSpan.End()
	}
